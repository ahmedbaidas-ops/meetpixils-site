...
function HighlightItem({
  ...
  asChild = false,
  forceUpdateBounds,
  // `render` support: motion-navigation-menu (and other newer registry items)
  // pass their element via a render prop rather than as a single child. This
  // installed primitive only understood the child-cloning API, so those
  // elements were silently dropped. Normalise both shapes here.
  render,
  ...props
}: Omit<HighlightItemProps, "children"> & { children?: React.ReactNode; render?: React.ReactElement }) {
  ...
  const renderEl = React.isValidElement<ExtendedChildProps>(render) ? render : null;
  // When `render` is given it is the real element. Only override its children
  // when this component actually received some: MotionNavigationMenuTrigger
  // passes label + chevron as children, but MotionNavigationMenuLink passes its
  // label inside the render element's own props and passes NO children — and
  // cloneElement(el, undefined, undefined) would replace those children with
  // nothing, blanking every link.
  const source: React.ReactNode = renderEl
    ? (React.Children.count(children) > 0
        ? React.cloneElement(renderEl, undefined, children)
        : renderEl)
    : children;
  const isValidChild = React.isValidElement<ExtendedChildProps>(source);
  const element = isValidChild ? (source as React.ReactElement<ExtendedChildProps>) : null;
  const childValue =
    id ??
    value ??
  ...
  if (!isValidChild || !element) return source;
